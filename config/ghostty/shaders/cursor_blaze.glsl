...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    vec4 cursorColor = vec4(iCurrentCursorColor.rgb, 1.0);
    vec4 cursorAccent = vec4(clamp(iCurrentCursorColor.rgb * 1.18, 0.0, 1.0), 1.0);
    ...
    // newColor = mix(newColor, cursorAccent, 1.0 - smoothstep(d2, -0.01, 0.001));
    // newColor = mix(newColor, cursorColor, 1.0 - smoothstep(d2, -0.01, 0.001));
    // newColor = mix(newColor, cursorColor, antialising(d2));

    float cCursorDistance = sdBox(vu, currentCursor.xy - (currentCursor.zw * offsetFactor), currentCursor.zw * 0.5);
    newColor = mix(newColor, cursorAccent, 1.0 - smoothstep(cCursorDistance, -0.000, 0.003 * (1. - progress)));
    newColor = mix(newColor, cursorColor, 1.0 - smoothstep(cCursorDistance, -0.000, 0.003 * (1. - progress)));
