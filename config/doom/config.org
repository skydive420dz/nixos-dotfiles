...
* Line Numbers
Keep visible line numbers for code, but remove the gutter from prose, notes, help, and file-management buffers.
...
* Responsiveness
Reduce small UI delays so typing, Corfu completions, LSP updates, and key-hint popups feel closer to Neovim.
...
* Workflow Polish Trial
Borrow a few small workflow ideas from daviwil's Emacs config while keeping them Doom-native and easy to revert.
...
Shape Doom's existing Consult and Projectile commands into a small NVF-like command layer.
