...
- `config/doom/config.org` is the readable source of truth. `config.el` is kept
  in sync from it for Doom.
- `C-h`, `C-j`, `C-k`, and `C-l` move focus between Emacs windows, including
  inside vterm insert state.
- Doom's native `SPC w ...` window map is intentionally left to Doom. Do not
  override `SPC w H/J/K/L` with custom swap/move helpers unless we explicitly
  decide to replace Doom's defaults.
...
- 2026-05-30: Doom Emacs experiment checkpoint: upstream Doom, repo-local live
