...
1. Enable =hardware.keyboard.qmk.enable=.
2. Remove the redundant explicit =qmk-udev-rules= system package.
3. Build and activate separately.
4. Reconnect the real device and verify its hidraw ACL, Vial discovery, and the
   intended configuration flow.
5. Decide separately whether the full QMK toolchain belongs system-wide or in
   an on-demand firmware project shell.  Firmware flashing is a distinct,
