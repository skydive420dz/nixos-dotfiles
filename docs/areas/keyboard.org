...
** Vial And QMK - Build Verified, Device Test Pending
:PROPERTIES:
...
1. Activate the build-verified slice separately.
2. Reconnect the real device and identify its USB and hidraw nodes.
3. Verify applied rules, active-seat ACL, and Vial discovery.
4. Decide separately whether the full QMK toolchain belongs system-wide or in
   an on-demand firmware project shell.  Firmware flashing remains a distinct,
