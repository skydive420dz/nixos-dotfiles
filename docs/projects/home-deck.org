...
The iPad is only the display. MSI owns the service, status collection, and Ollama proxy.

** Current Network Map
:PROPERTIES:
:CUSTOM_ID: current-network-map
:END:
...
:PROPERTIES:
:CUSTOM_ID: dd-wrt-static-leases
:END:
DD-WRT's static lease table was unreliable, so the stable entries live in *Services -> Services -> DNSMasq -> Additional DNSMasq Options*.
...
After editing, use *Save* then *Apply Settings*. Rebooting DD-WRT may be needed if a lease refuses to settle.

** iPad State
:PROPERTIES:
:CUSTOM_ID: ipad-state
:END:
...
Phoenix is semi-untethered. Packages and passwords persist, but if the iPad reboots, open Phoenix on the iPad and run the jailbreak again before expecting SSH to work.
...
Then use Safari's *Add to Home Screen* to run it as a near-fullscreen iOS web app. If the icon was created before the Apple web-app tags existed, delete and recreate it.

** Next Ideas
:PROPERTIES:
:CUSTOM_ID: next-ideas
:END:
- Router/DD-WRT panel
